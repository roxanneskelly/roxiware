        ...
        ActiveRecord::Base.transaction do
            begin
                ...
                if(@user.errors.blank?)
                    ...
                    auth_service_info.each do |provider, uid|
                        service = @user.auth_services.where(:provider=>provider).first
                        service.destroy if service.present?
                        if(uid.present?)
                            service = @user.auth_services.build
                            service.uid = uid
