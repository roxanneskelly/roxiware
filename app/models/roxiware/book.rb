class Roxiware::Book < ActiveRecord::Base
    ...
    def bookstores
        bookstores_param = get_param("bookstores")
        if bookstores_param.present?
            bookstores_param.a.collect{|bookstore| bookstore.to_jstree_data}
        else
            []
        end
    end
    ...
    before_validation do
        ...
        image_uri = URI(self.large_image) if self.large_image.present?
        if(image_uri.present? && !image_uri.host)
